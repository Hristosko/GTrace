...
private:
    explicit Matrix4x4(const __m128& a, const __m128& b, const __m128& c, const __m128& d) : rows{a, b, c, d} {}
...
inline __m256 twoRowsMult(const __m256& a01, const Matrix4x4& b)
{
    ...
    result = _mm256_add_ps(
        result, _mm256_mul_ps(_mm256_shuffle_ps(a01, a01, _MM_SHUFFLE(1, 1, 1, 1)), _mm256_broadcast_ps(&b.rows[1])));
    result = _mm256_add_ps(
        result, _mm256_mul_ps(_mm256_shuffle_ps(a01, a01, _MM_SHUFFLE(2, 2, 2, 2)), _mm256_broadcast_ps(&b.rows[2])));
    result = _mm256_add_ps(
        result, _mm256_mul_ps(_mm256_shuffle_ps(a01, a01, _MM_SHUFFLE(3, 3, 3, 3)), _mm256_broadcast_ps(&b.rows[3])));
    return result;
}
...
inline __m128 mult2x2(const __m128& a, const __m128& b)
{
    return _mm_add_ps(
        ...
        _mm_mul_ps(_mm_shuffle_ps(a, a, _MM_SHUFFLE(2, 3, 0, 1)), _mm_shuffle_ps(b, b, _MM_SHUFFLE(1, 2, 1, 2))));
}
...
inline __m128 multAdj2x2(const __m128& a, const __m128& b)
{
    return _mm_sub_ps(
        ...
        _mm_mul_ps(_mm_shuffle_ps(a, a, _MM_SHUFFLE(2, 2, 1, 1)), _mm_shuffle_ps(b, b, _MM_SHUFFLE(1, 0, 3, 2))));
}
...
inline Matrix4x4 Matrix4x4::inversed() const
{
    ...
    const __m128 det = _mm_sub_ps(
        _mm_mul_ps(
            _mm_shuffle_ps(rows[0], rows[2], _MM_SHUFFLE(2, 0, 2, 0)),
            _mm_shuffle_ps(rows[1], rows[3], _MM_SHUFFLE(3, 1, 3, 1))),
        _mm_mul_ps(
            _mm_shuffle_ps(rows[0], rows[2], _MM_SHUFFLE(3, 1, 3, 1)),
