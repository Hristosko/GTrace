...
public:
    ...
    Vector3f operator-() const { return _mm_sub_ps(_mm_setzero_ps(), this->vec); }
    ...
    bool operator>=(const Vector3f& a) const { return (a <= *this); }
    ...
    float lengthSqr() const { return dot(*this, *this); }

    float length() const { return sqrtf(this->lengthSqr()); }

    void sqrt() { this->vec = _mm_sqrt_ps(this->vec); }
